...
%So we have to bound P{\hat{ri} <= ri - 2si}. Till that time arm i will not be accepted as good arm. For arm j we have to bound P{\hat{rj} >= rj + 2sj} since till that time j will not be rejected as bad arm.

\end{itemize}

\begin{figure}
%\caption{AugUCB Intuition}
